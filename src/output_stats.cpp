...
namespace {
     ...
     *
     * The result is interpolated between the values immediately below
     * and above that fractional position.
     *
     * @param sorted_coverage Coverage values sorted in ascending order.
     * @param percentile Percentile expressed as a fraction in [0, 1].
     ...
     *
     * This avoids modifying the global locale of the application.
     */
    [[nodiscard]]
     ...
     *
     * This helper is private to output_stats.cpp and is placed
    ...
    void writeCoverageStatsSection(
        ...
        // Floating-point report values use fixed decimal notation.
        ...
        // Minimum and maximum are integral coverage values.
...
namespace output {
    CoverageStats computeCoverageStats(
        ...
        /*
         * Welford's online algorithm computes the mean and population
         * variance in one numerically stable pass.
         */
        ...
        // Median and quartiles require access to ordered values.
    ...
    void writeStatsReportQuery(
        ...
        // Required mapping statistics.
        constexpr std::array required_keys{"total", "mapped", "mapped_to_query", "unmapped"};
        for (const auto *key: required_keys) {
            ...
            const double pct_query_total = total
                                               ? 100.0 * static_cast<double>(mapped_to_query) / static_cast<double>(
                                                     total)
                                               : 0.0;
            const double pct_query_mapped = mapped
                                                ? 100.0 * static_cast<double>(mapped_to_query) / static_cast<double>(
                                                      mapped)
                                                : 0.0;
            const double pct_unmapped =
                    total ? 100.0 * static_cast<double>(unmapped) / static_cast<double>(total) : 0.0;

            // Open report.
            std::ofstream report(output_txt, std::ios::out);
            if (!report) {
    ...
    // TODO add the name to each compo
    void writeStatsReportGlobal(
        ...
        for (std::size_t component_id = 0; component_id + 1 < bp_component_offsets.size(); ++component_id) {
            ...
            std::vector<cdx::Coverage> component_cov; component_cov.reserve(component_end - component_start);
            ...
            writeCoverageStatsSection(report, "Coverage Statistics Component " +
                                              std::to_string(component_id), component_stats);
        }
    }
} // namespace output
